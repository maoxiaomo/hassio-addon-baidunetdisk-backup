...
### 改进
- **底部操作按钮固定在视口底部**：【重新加载】【保存并重启加载项】按钮改为 `position: fixed`，始终浮在浏览器底部并居中对齐内容区，不再随滚动消失；body 增加底部留白避免遮挡内容

## 1.2.2

### 改进
- **存储告警阈值改为百分比输入**：UI 输入 0-100 的整数（默认 90），写回 `options.json` 时自动转成 0-1 小数（即 90 → 0.9），后端逻辑/schema 不变
- **挪到事件区**：从【通知 — 全局】挪到【通知 — 事件开关】中"存储空间告警"勾选的紧下方，关系更直观

## 1.2.1

### 改进
- **合并 Web UI 标签页**：撤掉【通知测试】和【配置编辑】两个标签页，改为单页布局；每个通知渠道的【测试发送】按钮直接放在对应配置卡片的标题右侧，无需切换标签
- 保存按钮固定在底部 sticky 区域，方便长表单滚动后随时保存
